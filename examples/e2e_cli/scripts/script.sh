...
COUNTER=1
MAX_RETRY=5
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/productivist.com/orderers/orderer.productivist.com/msp/tlscacerts/tlsca.productivist.com-cert.pem
...
setGlobals () {
	...
	if [ $1 -eq 0 -o $1 -eq 1 ] ; then
		...
		if [ $1 -eq 0 ]; then
			CORE_PEER_ADDRESS=peer0.private.productivist.com:7051
		else
		...
		if [ $1 -eq 2 ]; then
			CORE_PEER_ADDRESS=peer0.public.productivist.com:7051
		else
			CORE_PEER_ADDRESS=peer1.public.productivist.com:7051
		fi
	fi
...
checkOSNAvailability() {
	...
	do
		 ...
		 if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
			 peer channel fetch 0 -o orderer.productivist.com:7050 -c "testchainid" >&log.txt
		 else
			 peer channel fetch 0 0_block.pb -o orderer.productivist.com:7050 -c "testchainid" --tls --cafile $ORDERER_CA >&log.txt
		 fi
...
# echo "Updating anchor peers for private..."
# updateAnchorPeers 0
...
#Invoke on chaincode on Peer0/private
echo "Sending invoke transaction on private/peer0..."
chaincodeInvoke 0
